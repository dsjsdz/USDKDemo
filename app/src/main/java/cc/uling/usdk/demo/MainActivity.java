    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clickView(R.id.btn_min_pay_amount, view -> {
            if (checkOpen()) {
                ...
                Log.i(TAG, para.toString());
                if (para.isOK()) {
                    addLog("the minimum payment unit amount: " + para.getValue() + ", decimal is: " + para.getDecimal());
                } else {
                    addError(para);
                }
            }
            ...
            if (checkOpen()) {
                ...
                if (para.isOK()) {
                    ...
                    addLog("Payment method is in " + name + "，and the amount received is " + para.getMultiple() + " times the minimum unit. " +
                            "pay type: " + para.getPayType() + ", status is " + para.getStatus() + ", cancel:" + para.getCancel() + ", fault:" + para.getFault());
                } else {
                    addError(para);
                }
            }
        ...
        clickView(R.id.btn_gzg_status, view -> {
            if (checkOpen()) {
                ...
                if (para.isOK()) {
                    addLog("addr is :"+ addr + " , box no is:" + para.getNo() + ", box status is : " + (para.getStatus() == 0 ? "opened." : "closed."));
                } else {
                    addError(para);
                }
            }
        ...
        clickView(R.id.btn_drop_status, view -> {
            if (checkOpen()) {
                ...
                if (para.isOK()) {
                    addLog("Shipping status is " + para.getStatus() + "," + (para.getStatus() == 0 ? "Drop detection is not connected or blocked" : "Drop detection is normal with no occlusion"));
                } else {
                    addError(para);
                }
            }
